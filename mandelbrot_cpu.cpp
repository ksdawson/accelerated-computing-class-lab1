...
void mandelbrot_cpu_vector(uint32_t img_size, uint32_t max_iters, uint32_t *out) {
    ...
    for (uint64_t i = 0; i < img_size; ++i) {
        ...
        __m512 cy_vector = _mm512_set1_ps((float(i) / float(img_size)) * 2.5f - 1.25f);
        for (uint64_t j = 0; j < vector_size; ++j) {
            // Get the plane coordinate cx for the image pixel.
            __m512 cx_vector = _mm512_set_ps(
                (float(j * 16 + 15) / float(img_size)) * 2.5f - 2.0f,
                (float(j * 16 + 14) / float(img_size)) * 2.5f - 2.0f,
                (float(j * 16 + 13) / float(img_size)) * 2.5f - 2.0f,
                (float(j * 16 + 12) / float(img_size)) * 2.5f - 2.0f,
                (float(j * 16 + 11) / float(img_size)) * 2.5f - 2.0f,
                (float(j * 16 + 10) / float(img_size)) * 2.5f - 2.0f,
                (float(j * 16 + 9) / float(img_size)) * 2.5f - 2.0f,
                (float(j * 16 + 8) / float(img_size)) * 2.5f - 2.0f,
                (float(j * 16 + 7) / float(img_size)) * 2.5f - 2.0f,
                (float(j * 16 + 6) / float(img_size)) * 2.5f - 2.0f,
                (float(j * 16 + 5) / float(img_size)) * 2.5f - 2.0f,
                (float(j * 16 + 4) / float(img_size)) * 2.5f - 2.0f,
                (float(j * 16 + 3) / float(img_size)) * 2.5f - 2.0f,
                (float(j * 16 + 2) / float(img_size)) * 2.5f - 2.0f,
                (float(j * 16 + 1) / float(img_size)) * 2.5f - 2.0f,
                (float(j * 16 + 0) / float(img_size)) * 2.5f - 2.0f
            );
